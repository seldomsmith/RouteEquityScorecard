...
### Mapbox GL & Next.js Client-Side Exception Prevention
- **Dynamic Import Enforcement (`ssr: false`)**: Mapbox GL JS relies on browser globals (`window`, `HTMLCanvasElement`, WebGL context). Static imports of Mapbox components in Next.js pages or scrollytelling wrappers trigger unhandled server-side rendering errors or hydration crashes (`Application error: a client-side exception has occurred`). All components importing `mapbox-gl` must be dynamically imported via `next/dynamic` with `{ ssr: false }`.
- **Async Callback & Cleanup Null-Safety**: Mapbox event listeners, `setTimeout` handlers, and async `fetch().then()` callbacks (such as isochrone GeoJSON loaders) can resolve after a component unmounts or during rapid state transitions. Always verify `map.current` / `mapRef.current` is non-null and check `map.current.getLayer(...)` before calling Mapbox API methods (`setPaintProperty`, `setLayoutProperty`, `fitBounds`). Check array bounds before accessing `coordinates[0][0]`.
